...
// at this link:
// https://github.com/desktop/desktop/blob/master/app/src/lib/git/diff.ts
// and it was mainly written by: https://github.com/

import * as Path from "path";
// import * as fileSystem from '../../lib/file-system'

import { getBlobBinaryContents } from "./show";
...
async function getImageDiff(
  ...
  if (file instanceof WorkingDirectoryFileChange) {
    ...
    if (file.status !== AppFileStatus.Deleted) {
      ...
      console.log("The functionality of reading an image from the" +
         " working dir has been taken away from git-drive/diff.ts ");
...
export async function getBlobImage(
  ...
  const contents = await getBlobBinaryContents(repository, commitish, path);
  const diff: Image = {
